...
fn create_branch_interactive(repo: &DieselFlowRepository) -> Result<(), Box<dyn Error>> {
    ...
    for (i, f) in flows.iter().enumerate() { println!("[{}] {} name={:?}", i, f.id, f.name); }
    ...
    if items.is_empty() { println!("El flow no tiene pasos aún. El cursor por defecto será 0"); }
    println!("Pasos (cursor):");
    for it in &items { println!("- {}: {}", it.cursor, it.key); }
...
fn delete_flow_interactive(repo: &DieselFlowRepository) -> Result<(), Box<dyn Error>> {
    ...
    } else { println!("Borrado cancelado"); }
    Ok(())
}
...
fn create_step_interactive(repo: &DieselFlowRepository) -> Result<(), Box<dyn Error>> {
    ...
    println!("El nuevo paso se añadirá al final del flujo en cursor={} (append-only)", cursor);
    ...
    let cmd_id = if cmd_id_s.trim().is_empty() { None } else { Some(Uuid::parse_str(cmd_id_s.trim()).map_err(|_| "command_id inválido")?) };
    let fd = FlowData { id: Uuid::new_v4(), flow_id: fid, cursor, key: key.trim().to_string(), payload: serde_json::from_str(&payload_s).unwrap_or(serde_json::json!(payload_s.trim())), metadata: serde_json::from_str(&metadata_s).unwrap_or(serde_json::json!(metadata_s.trim())), command_id: cmd_id, created_at: Utc::now() };
...
fn view_flow_full(repo: &DieselFlowRepository) -> Result<(), Box<dyn Error>> {
    ...
    let fid = flows[idx].id;
    let meta = repo.get_flow_meta(&fid).map_err(|e| Box::new(e) as Box<dyn Error>)?;
    println!("Flow {} name={:?} parent={:?} cursor={} version={}", meta.id, meta.name, meta.parent_flow_id, meta.current_cursor, meta.current_version);
    let items = repo.read_data(&fid, 0).map_err(|e| Box::new(e) as Box<dyn Error>)?;
    println!("--- pasos (cursor | key | payload) ---");
    for it in items { println!("{} | {} | {}", it.cursor, it.key, it.payload); }
    Ok(())
}
