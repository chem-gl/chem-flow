...
def fuse_molecules(
    smiles_a: str, smiles_b: str, atom_a: int, atom_b: int, bond_order: int = 1
) -> dict:
    """
    Fuses two molecules by creating a bond between atom_a (in first) and
    atom_b (in second).
    Parameters
    ----------
    smiles_a : str
    ...
    em.AddBond(
        int(atom_a),
        int(atom_b) + offset,
        _bond_type_from_order(int(bond_order)),
    )
    fused = em.GetMol()
