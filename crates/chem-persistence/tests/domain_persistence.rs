...
fn diesel_domain_persistence_family_lifecycle() {
    ...
    match repo.delete_molecule(m2.inchikey()) {
        ...
        other => panic!("expected validation error when deleting referenced molecule, got: {:?}",
                        other),
    }

    // Remove m2 from the family version id2 (creates id3)
    let id3 = repo.remove_molecule_from_family(&id2, m2.inchikey())
                  .expect("remove molecule from family");

    // The old version id2 still references m2, so delete_molecule should still fail
    match repo.delete_molecule(m2.inchikey()) {
        Err(DomainError::ValidationError(_)) => {}
        other => panic!("expected validation error after remove (old version exists), got: {:?}",
                        other),
    }
    ...
    repo.delete_molecule(m2.inchikey())
        .expect("delete molecule after removing family references");
