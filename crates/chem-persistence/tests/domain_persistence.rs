...
fn diesel_domain_persistence_family_lifecycle() {
  ...
  let m1 = Molecule::from_simple_parts("ABCDEFGHIJKLMN-OPQRSTUVWX-1",
                                       "CCO",
                                       "InChI=1S/C2H6O/c1-2-3/h3H,2H2,1H3",
                                       json!({})).expect("m1 create");
  let m2 = Molecule::from_simple_parts("ZYXWVUTSRQPONM-MLKJIHGFED-1",
                                       "CCN",
  ...
  // id2
  match repo.delete_molecule(m2.inchikey().as_str()) {
  ...
  let id3 = repo.remove_molecule_from_family(&id2, m2.inchikey().as_str()).expect("remove molecule from family");
  ...
  repo.delete_molecule(m2.inchikey().as_str()).expect("delete molecule after removing family references");
