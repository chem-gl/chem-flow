...
fn main() {
    ...
    let parent_cursor = 6;
    let new_flow_id = repo.create_branch(&flow_id,
                                         Some("rama1".into()),
                                         Some("queued".into()),
                                         parent_cursor,
                                         json!({}))
    ...
    let branch3 = repo.create_branch(&flow_id, Some("branch3".into()), Some("queued".into()), 3, json!({}))
                      .expect("branch3");
    ...
    println!("temp branch exists after delete: {}",
             repo.branch_exists(&temp_branch).unwrap());
