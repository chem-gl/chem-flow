...
fn main() {
  ...
  let parent_cursor = 6;
  let new_flow_id = repo.create_branch(&flow_id, parent_cursor, json!({})).expect("branch");
  ...
  let branch3 = repo.create_branch(&flow_id, 3, json!({})).expect("branch3");
  ...
  let temp_branch = repo.create_branch(&flow_id, 2, json!({})).expect("temp");
