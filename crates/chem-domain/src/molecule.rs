...
impl Molecule {
  ...
  fn new(inchikey: &str, smiles: &str, inchi: &str, metadata: serde_json::Value) -> Result<Self, DomainError> {
    ...
    Ok(Molecule { inchikey: normalized_inchikey, smiles: smiles.to_string(), inchi: inchi.to_string(), metadata })
  }
  ...
  pub fn from_smiles(smiles: &str) -> Result<Self, DomainError> {
    ...
    let engine = ENGINE.as_ref().map_err(|e| DomainError::ExternalError(format!("Motor químico no disponible: {}", e)))?;
