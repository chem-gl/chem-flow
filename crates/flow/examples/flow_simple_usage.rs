...
fn main() -> Result<(), FlowError> {
  ...
  let new_flow_id = engine.new_branch(&flow_id, parent_cursor, json!({}))?;
  ...
  let created_grand = engine.new_branch(&new_flow_id, grandparent_cursor, json!({}))?;
  ...
  let created_b3 = engine.new_branch(&flow_id, 3, json!({}))?;
  ...
  let temp_branch = engine.new_branch(&flow_id, 2, json!({}))?;
  ...
  let parent_for_prune = engine.new_branch(&flow_id, 4, json!({}))?;
  ...
  let child_of_parent = engine.new_branch(&parent_for_prune, 6, json!({}))?;
