...
use flow::domain::{FlowData, FlowMetadata};
...
#[tokio::test]
async fn create_branch_from_middle_point() -> Result<(), Box<dyn std::error::Error>> {
  ...
  let meta = FlowMetadata::new(Some("Main Flow".to_string()),
                               Some("active".to_string()),
                               None::<String>,
                               None::<uuid::Uuid>,
                               None::<i64>,
                               json!({ "description": "Main flow for branching test", "tags": ["main"], "main_param": "main_value" }));
  let flow_id = repo.create_flow(meta).await?;
  ...
  let flow_data3 = FlowData { id: Uuid::new_v4(),
                              ...
                              command_id: None,
                              created_at: chrono::Utc::now() };
  repo.persist_data(&flow_data1, 0).await?;
  repo.persist_data(&flow_data2, 1).await?;
  repo.persist_data(&flow_data3, 2).await?;
  // 4. Crear flujo ramificado con referencia al principal
  let branch_meta = FlowMetadata::new(Some("Branch Flow".to_string()),
                                      Some("active".to_string()),
                                      None::<String>,
                                      None::<uuid::Uuid>,
                                      None::<i64>,
                                      json!({ "description": "Branched from main flow at step2", "tags": ["branch"], "branch_param": "branch_value", "parent_flow_id": flow_id.to_string(), "parent_cursor": 2 }));
  let branch_id = repo.create_flow(branch_meta).await?;
  ...
  let branch_data2 = FlowData { id: Uuid::new_v4(),
                                ...
                                command_id: None,
                                created_at: chrono::Utc::now() };
  repo.persist_data(&branch_data1, 0).await?;
  repo.persist_data(&branch_data2, 1).await?;
  // 6. Verificar el flujo principal
  let main_meta = repo.get_flow_metadata(&flow_id).await?;
  let main_steps = repo.read_data(&flow_id, 0).await?;
  ...
  // 7. Verificar la rama
  let branch_meta = repo.get_flow_metadata(&branch_id).await?;
  let branch_steps = repo.read_data(&branch_id, 0).await?;
  ...
  let parent_id = repo.get_metadata(&branch_id, "parent_flow_id").await?;
