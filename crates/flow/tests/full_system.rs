...
fn full_flow_lifecycle_and_branching() {
    ...
    // create a flow
    let flow_id = engine.start_flow(Some("root".into()), Some("queued".into()), json!({}))
                        .expect("create flow");
    ...
    let child = engine.new_branch(&flow_id, Some("child".into()), Some("queued".into()), 3, json!({}))
    ...
    let res = engine.append(child, "Step", json!({"child":1}), json!({}), None, 0)
                    .expect("append child");
    match res {
    ...
    let grand = engine.new_branch(&child, Some("grand".into()), Some("queued".into()), 4, json!({}))
                      .expect("create grand");
...
fn persist_idempotency_and_conflict() {
    ...
    let flow_id = engine.start_flow(Some("idemp".into()), Some("queued".into()), json!({}))
                        .expect("create");

    // append first step with a command_id
    let cmd = Uuid::new_v4();
    let r1 = engine.append(flow_id, "Step", json!({"v":1}), json!({}), Some(cmd), 0)
                   .expect("append1");
    ...
    let r2 = engine.append(flow_id, "Step", json!({"v":1}), json!({}), Some(cmd), current_version)
                   .expect("append2");
    match r1 {
    ...
    let bad_data = FlowData { id: Uuid::new_v4(),
                              ...
                              command_id: None,
                              created_at: Utc::now() };
    let res = engine.persist_data(&bad_data, meta.current_version + 1)
                    .expect("persist_data");
    match res {
...
fn snapshots_save_and_load_meta_behaviour() {
    ...
    let flow_id = engine.start_flow(Some("snap".into()), Some("queued".into()), json!({}))
                        .expect("create");
    // append a step
    engine.append(flow_id, "Step", json!({"a":1}), json!({}), None, 0)
          .expect("append");

    // save snapshot metadata
    let snap_id = engine.save_snapshot(&flow_id, 1, "state-1", json!({"k": "v"}))
