...
pub trait ChemicalFlowEngine: Send + Sync {
    ...
    fn engine_workflow_type() -> WorkflowType where Self: Sized;
    ...
    fn rehydrate(
        ...
        flow_repo: Arc<dyn FlowRepository>,
        domain_repo: Arc<dyn DomainRepository>
    ) -> Result<Self, WorkflowError> where Self: Sized {
    ...
    fn current_step(&self) -> u32 {
        self.extract_metadata_field("current_step")
            .and_then(|v| v.as_u64())
    ...
    fn execute_current_step_typed<I: serde::Serialize>(
        &mut self,
        input: &I
    ) -> Result<StepInfo, WorkflowError> {
    ...
    fn get_last_step_payload(&self, step_name: &str) -> Result<Option<JsonValue>, WorkflowError> {
        ...
        let payload = data
            .into_iter()
            .rev()
            .find(|fd| fd.key.eq_ignore_ascii_case(&key))
            .map(|fd| fd.payload);
    ...
    fn get_metadata(&self, key: &str) -> Result<JsonValue, WorkflowError> {
        self.flow_repo()
            .get_meta(&self.id(), key)
            .map_err(|e| WorkflowError::Persistence(format!("get_meta error: {}", e)))
    }
    ...
    fn validate_step_execution(&self, step_name: &str) -> Result<(), WorkflowError> {
        ...
        if self.get_last_step_payload(step_name)?.is_some() {
            return Err(WorkflowError::Validation(
                format!("El paso '{}' ya fue ejecutado para este flow", step_name)
            ));
        }

        // Verificar dependencias de pasos previos
        let required_steps: Vec<String> = (0..self.current_step())
    ...
    fn ensure_previous_steps_present(&self, required: &[String]) -> Result<(), WorkflowError> {
        let missing: Vec<String> = required
            .iter()
            .filter(|req| self.get_last_step_payload(req).ok().flatten().is_none())
            .cloned()
            .collect();
        ...
        if missing.is_empty() {
            ...
            Err(WorkflowError::Validation(
                format!("Datos faltantes de pasos previos: {:?}", missing)
            ))
        }
    }
    ...
    fn calculate_cursor_and_version(&self, expected_version: i64) -> Result<(i64, i64), WorkflowError> {
        self.flow_repo()
            ...
            .map(|meta| {
                let version = if expected_version < 0 {
    ...
    fn determine_current_step_from_data(&self) -> Result<u32, WorkflowError> {
        ...
        self.flow_repo()
            .get_flow_meta(&self.id())
            .map(|meta| (meta.current_cursor as u32).saturating_add(1))
            .or(Ok(0))
    }
    ...
    fn extract_metadata_field(&self, field: &str) -> Option<JsonValue> {
        self.get_metadata("flow_metadata")
            .ok()
            .and_then(|meta| meta.get(field).cloned())
    }
    ...
    fn update_metadata_field(&mut self, field: &str, value: JsonValue) -> Result<(), WorkflowError> {
        let mut metadata = self.extract_metadata_field("flow_metadata")
